...
def generate_city_candidates(city):
    """根据用户选择的城市名生成可能的候选列表"""
    if not isinstance(city, str):
        return [city]
    candidates = [city]
    # 去除所有空格
    no_space = city.replace(' ', '').replace('　', '')
    candidates.append(no_space)
    # 去除“市”字
    if city.endswith('市'):
        candidates.append(city[:-1])
    # 去除空格和“市”
    if no_space.endswith('市'):
        candidates.append(no_space[:-1])
    # 添加带一个空格的变体（模拟可能的误输入）
    if ' ' not in city and len(city) > 1:
        candidates.append(city[0] + ' ' + city[1:])
    # 去重并保持顺序
    seen = set()
    unique_candidates = []
    for c in candidates:
...
def predict_price_with_city_fallback(full_input_dict, city_candidates=None):
    """
    使用城市候选列表进行预测，返回 (预测价格, 实际使用的城市)
    若所有候选都失败，抛出 ValueError
    """
    original_city = full_input_dict.get('城市', '')
    if city_candidates is None:
        city_candidates = generate_city_candidates(original_city)
    else:
        city_candidates = list(city_candidates)
        # 确保包含原始城市
        if original_city not in city_candidates:
            city_candidates.insert(0, original_city)

    last_error = None
    for city in city_candidates:
        temp_dict = full_input_dict.copy()
        temp_dict['城市'] = city
        try:
            pred = predict_price(temp_dict)  # 调用原有的 predict_price 函数
            return pred, city
    ...
    # 所有候选失败
    raise ValueError(f"无法使用任何城市候选进行预测。原始错误：{last_error}")

def probe_allowed_cities(candidate_list, base_input_dict):
    """
    探测模型可接受的城市名称列表。
    candidate_list: 待测试的城市名称列表
    base_input_dict: 一个完整的、具有合理数值的特征字典（用于构造预测输入）
    返回：可识别的城市列表
    """
    allowed = []
