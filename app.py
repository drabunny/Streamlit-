...
def encode_categorical(value, encoder):
    """对分类变量进行编码，若值不在原有类别中则映射为'其他'"""
    try:
        return encoder.transform([value])[0]
    except ValueError:
        # 若类别不在训练集中，尝试用'其他'（如果存在）否则用第一个类别
...
def plot_shap_waterfall(input_dict):
    ...
    shap.waterfall_plot(
        ...
        show=False,
        max_display=10  # 只显示最重要的10个特征，避免图过于拥挤
    )
    plt.tight_layout()
    return plt
...
if use_macro:
    income = st.sidebar.number_input("城镇居民人均可支配收入 (元)", min_value=30000, max_value=100000, value=62506,
                                     step=1000)
...
with col_right:
    ...
    if predict_btn:
        ...
        # 构造输入字典（必须与 FEATURE_COLS 顺序严格一致）
        input_dict = {
            '建筑面积': area,
            '房龄': age,
            '朝向': ori_encoded,
...
st.markdown("---")
st.caption(
    "注：预测基于 XGBoost 模型（2021-2024年山东省济南/烟台/济宁数据），SHAP瀑布图显示各特征对预测值的影响贡献。宏观指标默认使用济南市2023年数据，可在侧边栏自定义。")